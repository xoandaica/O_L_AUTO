...
String wifiPW_init = GlobalVariable.wifi_pw
println('Wi-Fi name init: ' + wifiName_init + ' -- Wi-Fi password init: ' + wifiPW_init)

//----------------------------------------Tạo file backup config----------------------------------------
Mobile.callTestCase(findTestCase('Test Cases/Device control/Backup Config/OL_103 Create Backup file success'), [('newSessionBkCreate') : 'false'], FailureHandling.STOP_ON_FAILURE)
String fileBkup_name = GlobalVariable.fileBkup_name
println('Tên file backup: ' + fileBkup_name)

//----------------------------------------Chỉnh sửa wifi----------------------------------------
Mobile.callTestCase(findTestCase('Test Cases/Dashboard/OL_28 Update wifi infor success'), [('newSessionEditWifi') : 'false'], FailureHandling.STOP_ON_FAILURE)
String wifiName_after = GlobalVariable.wifi_name
String wifiPW_after = GlobalVariable.wifi_pw
...
//----------------------------------------Restore config----------------------------------------
Mobile.callTestCase(findTestCase('Test Cases/Device control/Restore/OL_96 Access Restore screen success'), [('newSessionRestore') : 'false'], FailureHandling.STOP_ON_FAILURE)
...
TestObject btn_refresh = findTestObject('Object Repository/Mesh/Device/Device control/Restore config/btn_refresh')
Mobile.callTestCase(findTestCase('Test Cases/Dashboard/OL_23 View wifi info success'), [('newSessionViewWifi') : 'false'], FailureHandling.STOP_ON_FAILURE)

String wifiName_restore = GlobalVariable.wifi_name
String wifiPW_restore = GlobalVariable.wifi_pw
println('Wi-Fi name init: ' + wifiName_init + ' -- Wi-Fi password init: ' + wifiPW_init)
println('Wi-Fi name after: ' + wifiName_after + ' -- Wi-Fi password after: ' + wifiPW_after)
